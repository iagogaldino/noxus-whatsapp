...
const EmployeeList: React.FC = () => {
  ...
  const { searchEmployees, deleteEmployee, isLoading, error } = useEmployees();
  ...
  const handleDelete = async () => {
    ...
    if (result.success) {
      ...
      setDeleteError(result.error ?? 'Falha ao excluir funcionário.');
    }
  };
                ...
                <IonList className="admin-list">
                  {filtered.map((employee) => (
                    <EmployeeListItem
                      key={employee.id}
                      employee={employee}
                      onEdit={(employeeId) => history.push(`/admin/employees/${employeeId}/edit`)}
                  ...
                  <tbody>
                    {filtered.map((employee) => (
                      <tr key={employee.id}>
                        ...
                        <td>{formatPhoneLabel(employee.phone)}</td>
